...
class DMSBaseModel(models.BaseModel):
    ...
    def __print_values(self, values):
        ...
        for key, value in values.iteritems():
            if key == 'filename':
                print_vals[key] = value.encode("utf-8")
            else:
    ...
    def _validate_filename_length(self,filename):
        """ 校验上传的文件名长度是否大于五十 """
        if len(filename) > 50:
            raise AccessError(_("The file name is too long to be greater than 50."))
    
    @api.model
    def create(self, vals):
        # 校验文件名长度是否大于五十
        if vals.get('filename'):
            self._validate_filename_length(vals.get('filename'))
    ...
    def _check_dup_file(self,vals):
        """
        校验当前文件内容是否和上传的文件一样
        :param values:
        :return:
        """
        if "file" in vals and self.file_ref.exists():
            cur_checksum=self.file_ref.checksum
            file = base64.decodestring(vals['file'])
            new_checksum=hashlib.sha1(file).hexdigest()
    ...
    def write(self, vals):
        ...
        #校验当前文件是否和上传文件内容一样，如果内容一样不上传文件,避免重复提交文件
        check_result=self.sudo()._check_dup_file(vals)
        ...
        for record in self:
            ...
            vals = record._append_values_write(vals)
    ...
    def unlink(self):     
        ...
        result = super(DMSBaseModel, self).write({'active': False})# 打失效标记，非物理删除
        self._after_unlink()
        return result
