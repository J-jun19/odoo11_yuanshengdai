...
class IacSpotVendor(models.Model):
    ...
    #PW add transfer_number 2018 10 08, interface config need add this fields to SAP
    ...
    @api.constrains('bank_country')
    def _check_bank_country(self):
        for record in self:
            lc_plant = self.env['pur.org.data'].browse(record.plant.id)
            if lc_plant.plant_code == 'TP02' and not record.transfer_number:
                raise ValidationError(_(u'台湾必须填写Bank Transfer Number！/ Bank Transfer Number is required for Taiwanese Vendor！'))

            # if country.code != 'TW' and record.bank_country!=False:
            #     raise UserError(_(u'非台湾不能填写Bank Transfer Number！/ Bank Transfer Number is required for none Taiwanese Vendor!'))
    ...
    def _onchange_plant(self):
        ...
        elif self.plant.plant_code == 'CP20':
            user_id = self.env['res.users'].search([('login', '=', 'SPOTIACS')])
            if not user_id:
                raise UserError(_(u'系统未设置SPOTIACT用户，请联系管理员检查用户资料！'))
            else:
                self.user_id = user_id.id

        elif self.plant.plant_code == 'CP29':
            user_id = self.env['res.users'].search([('login', '=', 'SPOTIANB')])
            if not user_id:
                raise UserError(_(u'系统未设置SPOTIACT用户，请联系管理员检查用户资料！'))
            else:
                self.user_id = user_id.id
            ...
            if not context["approve_status"] or not context.get("data") or not context.get("data").get("id"):
                ...
                if context["approve_status"] and context["rpc_callback_data"]["FormStatus"] == "C":
                    ...
                    if not model_obj.exists():
                       err_msg="id is %s not exists in iac.spot.vendor "% (context.get("data").get("id"),)
    ...
    def button_to_sap(self, object_id=None):
        ...
        if rpc_result:
            ...
            vendor_reg_vals = {
                ...
                #add by PW 2018 10 09 for SPOT/MOLD/BVI new field bank transfer number
    ...
    def _check_bank_country(self):
        for record in self:
            ...
            if lc_plant.plant_code == 'TP02' and not record.transfer_number:
                raise ValidationError(_(u'台湾必须填写Bank Transfer Number！/ Bank Transfer Number is required for Taiwanese Vendor！'))
            #
            # if country.code != 'TW' and record.transfer_number!=False:
            #     raise UserError(_(u'非台湾不能填写Bank Transfer Number！/ Bank Transfer Number is required for none Taiwanese Vendor!'))
