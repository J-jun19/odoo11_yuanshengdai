...
class IacVendor(models.Model):
    ...
    class_date = fields.Date('Class Date', related='supplier_company_id.class_date', readonly=True,
                             copy=False)  # 评核日期，每次评核生效时更新该字段
    score_snapshot = fields.Char(string='Score Snapshot', related='supplier_company_id.score_snapshot', readonly=True,
                                 copy=False)  # 评核快照号
    ...
    transfer_number = fields.Char(string="Bank Number(TaiwanSupplier Must Specify)")
    bank_street = fields.Char(string="Bank - Street*", size=35)
    bank_city = fields.Char(string="Bank - City*", size=35)
    bank_country = fields.Many2one('res.country', string="Bank - Country Code *",
                                   domain=['&', ('show_in_list', '=', 'Y'), ('sh_import', '=', 'N')])
    ...
    destination = fields.Char(string="Destination*", size=28)
    ...
    site_survey = fields.Char(string="Site Survey")  # 是否现地评鉴
    comment = fields.Char(string="Comment")  # Webflow 返回的comments 信息
    ...
    # 新加的字段记录vendor_copy_id
    vendor_copy_id = fields.Integer(string="Vendor Copy id value")
    ...
    def _compute_buyer_id(self):
        for vendor in self:
            if vendor.buyer_email:
                buyer = self.env['res.partner'].search([('active', '=', True), ('email', '=', vendor.buyer_email)],
                                                       limit=1)
    ...
    def button_to_approve(self):
        ...
        if not self.local_foreign:
            raise UserError(_(u'內外購屬性不能为空！/ Local or Foreign is required!'))

        if self.plant.plant_code == 'TP02' and self.local_foreign == 'local' and not self.vendor_reg_id.vat_number:
            raise UserError(_(u'VAT沒有填寫的廠商不能選local！/ Vendor without VAT in F01 must be foreign !'))
    ...
    def vendor_be_normal_callback(self, context=None):
        ...
        try:
            # 校验接口入参
            if not context["approve_status"]:
                proc_result = False
                proc_ex.append(u"接口调用参数异常,approve_status 字段不存在")
                return proc_result, proc_ex
            if not context["data"]:
                proc_result = False
                proc_ex.append(u"接口调用参数异常,data 字段不存在")
                return proc_result, proc_ex
            if not context.get("data").get("id"):
                proc_result = False
                proc_ex.append(u"接口调用参数异常,data//id  字段不存在")
                return proc_result, proc_ex
            ...
            if context["approve_status"] and context["rpc_callback_data"]["FormStatus"] == "C":
                ...
                # 更新附件档案状态 写入memo
                file = self.env['iac.vendor.attachment'].search(
                    [('group', '=', 'bank'), ('vendor_id', '=', vendor['id'])])
                for update_file in file:
                    update_file.write(
                        {'state': 'active', 'memo': 'webflow单号：%s 签核通过！' % context["rpc_callback_data"]["EFormNO"]})

                # add by PW 2018-9-13,F02暫時沒有回傳文件
                # 签核通过的情况下更新附件档案状态
                # file_list = context.get("rpc_callback_data").get("params").get("File")
                # for file_val in file_list:
                #     file_id = int(file_val.get("File_ID", False))
                #     exp_date_str = file_val.get("ExpirationDate", False)
                #     memo = file_val.get("Memo", False)
                #     expiration_date = False
                #     if exp_date_str != False and len(exp_date_str) > 0:
                #         expiration_date = time.strftime('%Y-%m-%d', time.strptime(exp_date_str, '%Y/%m/%d'))
                #     file_attachment_rec = self.env["iac.vendor.attachment"].browse(file_id)
                #     if not file_attachment_rec.exists():
                #         err_msg = "id is %s not exists in iac.vendor.attachment " % (file_id,)
                #         proc_ex.append(err_msg)
                #         proc_result = False
                #         return proc_result, proc_ex
                #     update_file_vals = {
                #         "state": "active",
                ...
                #         "memo": memo
                #     }
                #     file_attachment_rec.write(update_file_vals)
                ...
                vendor = self.browse(context.get("data").get("id"))
                # 签核没有通过的情况下更新附件档案状态 写入memo
                file = self.env['iac.vendor.attachment'].search(
                    [('group', '=', 'bank'), ('vendor_id', '=', vendor['id'])])
                for update_file in file:
                    update_file.write(
                        {'state': 'inactive', 'memo': 'webflow单号：%s 签核未通过！' % context["rpc_callback_data"]["EFormNO"]})
                # 签核没有通过的情况下更新附件档案状态
                # file_list = context.get("rpc_callback_data", {}).get("params", {}).get("File", [])
                # for file_val in file_list:
                #     file_id = int(file_val.get("File_ID", False))
                ...
                #         "state": "inactive",
                #         "expiration_date": expiration_date,
                #         "expiratimemoon_date": memo
                #     }
                #     file_attachment_rec.write(update_file_vals)

                vendor.write(
                    {'state': 'unapproved', 'state_msg': u'webflow(%s)签核未通过' % context["rpc_callback_data"]["EFormNO"]})
...
class IacSupplierCompany(models.Model):
    ...
    supplier_type = fields.Selection(string='Supplier Company Type *', selection='_selection_supplier_type',
                                     required=True)
    line_ids = fields.One2many("iac.supplier.company.line", 'supplier_company_id', required=True, string="Vendors")
    comment = fields.Text(string="Comments")
    current_class = fields.Selection([('A', 'A'), ('B', 'B'), ('C', 'C'), ('D', 'D'), ('DW', 'DW')], default='C',
                                     readonly=True)
...
class IacSupplierCompanyLine(models.Model):
    ...
    state_msg = fields.Text(string='State Message')
    state = fields.Selection([('draft', 'Draft'), ('done', 'Done'), ('sap_error', 'SAP Error')], string="Status")
    vendor_reg_id = fields.Many2one("iac.vendor.register", related="vendor_id.vendor_reg_id", readonly=True)
    supplier_type = fields.Selection(string="Supplier Type", related="vendor_reg_id.supplier_type", readonly=True)
...
class IacGlobalVendorLine(models.Model):
    ...
    supplier_company_id = fields.Many2one('iac.supplier.company', string="Supplier Company", required=True,
                                          domain=[('is_bind', '=', False)])
...
class IacVendorCopy(models.Model):
    ...
    ori_vendor_id = fields.Many2one('iac.vendor', string="Copy Vendor", required=True,
                                    domain=[('state', 'in', ['done', 'block']), ('vendor_type', '=', 'normal')],
                                    index=True)
    ori_company_name = fields.Char(related="ori_vendor_id.name", string="Company Name")
    ori_local_foreign = fields.Selection([("local", "Local"), ("foreign", "Foreign")],
                                         related="ori_vendor_id.local_foreign", string="Local or Foreign")
    ...
    # add by PW 2018 10 08
    ori_address_country = fields.Many2one('res.country', related="ori_vendor_id.vendor_reg_id.address_country",
                                          string="Address country", readonly=True)
    ...
    # 20181129
    # Hi, Pei - Wu:    #
    # 我剛剛講的不太對，應該是說    TP02 and Country = TW，VATNumber必填才對。
    # 因為在上海跟南京會跟TW Vendor  交易，但不需要有  VAT   Number。
    # Regards,
    # Jocelyn   Lai
    ...
    def _check_vat_number(self):
        for record in self:
            lc_plant = self.env['pur.org.data'].browse(record.plant.id)
            # lc_country = self.env['res.country'].browse(record.ori_address_country.id)
            if lc_plant.plant_code == 'TP02' and record.local_foreign == 'local' and not record.vat_number:
    ...
    def create(self, values):
        ...
        same_count = self.sudo().search_count(
            [('plant', '=', values.get('plant', False)), ('currency', '=', values.get('currency', False)),
             ('buyer_email', '=', values.get('buyer_email', False)),
             ('payment_term', '=', values.get('payment_term', False)),
             ('incoterm', '=', values.get('incoterm', False)), ('destination', '=', values.get('destination', False)),
             ('local_foreign', '=', values.get('local_foreign', False)),
             ('ori_vendor_id', '=', values.get('ori_vendor_id', False)), ('state', '!=', 'cancel')])
        if same_count > 0:
    ...
    def _onchange_ori_vendor_id(self):
        ...
        if self.ori_vendor_id:
            self.env.cr.execute("select address_country,vat_number from iac_vendor_register where vendor_id=%s",
                                (self.ori_vendor_id.id,))

            vendor_infos = self.env.cr.fetchall()

            vars = {
                'currency': self.ori_vendor_id.currency.id,
                'payment_term': self.ori_vendor_id.payment_term.id,
                'incoterm': self.ori_vendor_id.incoterm.id,
                'destination': self.ori_vendor_id.destination,
                'local_foreign': self.ori_vendor_id.local_foreign,

                # add by PW 2018 10 08
                'ori_address_country': vendor_infos[0][0],
                'vat_number': vendor_infos[0][1]
            }
    ...
    def button_to_approve(self):
        ...
        if self.sudo().ori_vendor_id.local_foreign != self.local_foreign:
            change_flag = True
        # add by PW 20181108 begin
        if self.sudo().ori_plant.id != self.plant.id:
            change_flag = True
        # add by PW 20181108 end
        if not change_flag:
        ...
        same_count = self.search_count([('plant', '=', self.plant.id), ('currency', '=', self.currency.id),
                                        ...
                                        ('ori_vendor_id', '=', self.ori_vendor_id.id),
                                        ('id', '!=', self.id),
                                        ('state', 'in', ('to approve', 'unapproved', 'to sap', 'sap error', 'done'))])
        if same_count > 0:
    ...
    def vendor_copy_callback(self, context=None):
        ...
        try:
            ...
            if not context["approve_status"] or not context.get("data") or not context.get("data").get("id"):
                ...
                # 根据返回的id判断vendor copy对象是否为done状态,如果是done状态则报错
                if model_obj.state == 'done':
                    proc_result = False
                    proc_ex.append(u"id为%s的vendor copy已经有webflow单号签核完毕，请勿重复送件" % (context.get("data").get("id"),))
                    return proc_result, proc_ex
                ...
                if context["approve_status"] and context["rpc_callback_data"]["FormStatus"] == "C":
                    ...
                    self.record_callback_log('F03_E', webflow_number, 'new_vendor_register')
                    ...
                    self.record_callback_log('F03_E', webflow_number, 'new_product')
                    ...
                    self.record_callback_log('F03_E', webflow_number, 'new_factory')
                    factory_ids = []
                    ...
                    self.record_callback_log('F03_E', webflow_number, 'new_register_attachment')
                    ...
                    for ori_attachment in model_obj.ori_vendor_id.vendor_reg_id.attachment_ids:
                        ...
                        try:
                            ...
                            attachment_val = {
                                ...
                                'upload_date': ori_attachment.upload_date
                            }
                            ...
                            sys_error = traceback.format_exc()
                            # user_error="Copy File Failed,File id is %s ,File Name is %s "%(ori_attachment.file_id.id,ori_attachment.file_id.filename)
                            user_error = "Copy File Failed,File id is %s ,File Name is %s,System Error is %s " % (
                                ori_attachment.file_id.id, ori_attachment.file_id.filename, sys_error)
                            raise UserError(user_error)

                    new_vendor_reg_vals['attachment_ids'] = attachment_ids

                    self.record_callback_log('F03_E', webflow_number, 'new_materials')
                    ...
                    new_vendor_reg_obj = self.env['iac.vendor.register'].with_context(
                        {"no_check_short_name": True}).create(new_vendor_reg_vals)
                    # 补充缺少的附件栏位信息

                    self.record_callback_log('F03_E', webflow_number, 'fill_blank_register_attachment')

                    new_vendor_reg_obj.fill_blank_attachment()

                    self.record_callback_log('F03_E', webflow_number, 'New_vendor_data')
                    ...
                    self.record_callback_log('F03_E', webflow_number, 'new_vendor_attachment_1')

                    new_vendor_vals = model_obj.ori_vendor_id.copy_data(new_vendor_vals)[0]

                    self.record_callback_log('F03_E', webflow_number, 'new_vendor_attachment_2')
                    attachment_ids = []
                    ...
                    # 补充缺少的附件栏位信息
                    self.record_callback_log('F03_E', webflow_number, 'fill_blank_vendor_attachment')
                    new_vendor_obj.fill_blank_attachment()

                    # 放置当前vendor 到supplier_company_line中
                    # iac.supplier.company.line
                    self.record_callback_log('F03_E', webflow_number, 'extend_supplier_company')
                    if model_obj.ori_vendor_id.supplier_company_id.exists():
                    ...
                    self.record_callback_log('F03_E', webflow_number, 'write_vendor_register')
                    new_vendor_reg_obj.write({'vendor_id': new_vendor_obj.id})
                    self.record_callback_log('F03_E', webflow_number, 'write_vendor')
                    model_obj.write({'new_vendor_reg_id': new_vendor_reg_obj.id, 'new_vendor_id': new_vendor_obj.id})
                    # 更新vendor.material
                    self.record_callback_log('F03_E', webflow_number, 'write_vendor_materials')
                    for material in new_vendor_reg_obj.material_ids:
                        material.write({'vendor_id': new_vendor_obj.id})

                    # 调用接口之前提交事务保存数据
                    self.record_callback_log('F03_E', webflow_number, 'commit_transaction')
                    self.env.cr.commit()
                    ...
                    # 调用SAP接口多线程模式
                    self.record_callback_log('F03_E', webflow_number, 'start_new_thread_send_to_sap')
                    vendor_copy_vals = {
                        "vendor_copy_id": context.get("data").get("id"),
                    }
                    vendor_new_call_sap_thread = threading.Thread(target=self.vendor_new_call_sap,
                                                                  kwargs=vendor_copy_vals)
                    vendor_new_call_sap_thread.start()
                else:
                    model_obj = self.browse(context.get("data").get("id"))
                    model_obj.write({'state': 'unapproved',
                                     'state_msg': u'webflow(%s)签核未通过' % context["rpc_callback_data"]["EFormNO"]})

            self.record_callback_log('F03_E', webflow_number, 'return_values')
    ...
    @odoo_env
    def vendor_new_call_sap(self, **kwargs):
        """
        只能被vendor_copy记录对象调用,单笔记录发送到SAP系统
        """
        vendor_copy_id = kwargs.get("vendor_copy_id", False)
        vendor_copy_rec = self.env["iac.vendor.copy"].browse(vendor_copy_id)
        if not vendor_copy_rec:
            ex_msg = "model name is iac.vendor.copy ,rec id is {} does not exists!".format(vendor_copy_id)
            raise UserError(ex_msg)

        vendor = vendor_copy_rec.new_vendor_id
        if not vendor.vendor_reg_id.exists():
            ex_msg = "model name is iac.vendor ,rec id is {};vendor_reg_id value is  invalid!".format(vendor.id)
            vendor_ex_vals = {
                'state': 'sap error',
                'state_msg': u'通知SAP失败,异常信息为 %s' % (ex_msg,)
            }
            vendor_copy_rec.write(vendor_ex_vals)
            self.env.cr.commit()
            raise UserError(ex_msg)
        if vendor.state not in ['to sap', 'sap error']:
            ex_msg = "model name is iac.vendor ,rec id is {} ;state is invalid,not in  ['to sap', 'sap error']!".format(
                vendor.id)
            vendor_ex_vals = {
                'state': 'sap error',
                'state_msg': u'通知SAP失败,异常信息为 %s' % (ex_msg,)
        ...
        try:
            ...
            if rpc_result:
                ...
                # 修改vendor copy单子状态
                vendor_copy_rec.write({'state': 'done',
                                       'state_msg': u'通知SAP成功'})
                return True
            else:
                # 修改vendor_reg状态
                ...
                # 记录异常信息
                vendor.write(vendor_ex_vals)
                vendor.vendor_reg_id.write(vendor_ex_vals)
                vendor_copy_rec.write(vendor_ex_vals)
                self.env.cr.commit()
        ...
        try:
            if vendor.state in ('to sap', 'sap error'):
                ...
                if rpc_result:
                    ...
                    if vendor.vendor_reg_id.exists():
                        vendor.vendor_reg_id.write(vendor_ex_vals)
                        self.env.cr.commit()
                    else:
                        self.env.cr.commit()
                        raise UserError("Vendor  %s has no Vendor Register Info" % (vendor.vendor_code))
                    return False
            else:
                self.write({'state': 'done'})
                return True
    ...
    def record_callback_log(self, int_no, webflow_number, action_type):
        """
        记录vendor copy call back 的每一步动作，传入参数有三个
        接口编号，webflow no，动作
        """
        self._cr.execute("""insert into iac_webflow_call_back_log (int_no, webflow_no ,action_type )
          VALUES 
        (%s,%s,%s)""", (int_no, webflow_number, action_type))
...
class IacVendorBlock(models.Model):
    ...
    @api.model
    def create(self, values):
        vendor_block_ids = self.sudo().search(
            [('vendor_id', '=', values['vendor_id']), ('is_block', '=', values['is_block']), ('state', '=', 'draft')])
        if vendor_block_ids:
    ...
    def _check_vendor_id(self):
        for record in self:
            ...
            # vendor block/unblock/change流程中的不能送签表单
            vendor_block_ids = record.env['iac.vendor.block'].search([('vendor_id', '=', record.vendor_id.id), (
                'state', 'in', ('to approve', 'unapproved', 'to sap', 'sap error'))])
            if vendor_block_ids:
                raise ValidationError(_(u'Vendor已经有block/unblock流程中的单子！'))
            change_basic_ids = record.env['iac.vendor.change.basic'].search(
                [('vendor_reg_id', '=', record.vendor_id.vendor_reg_id.id),
                 ('state', 'in', ('to approve', 'unapproved', 'to sap', 'sap error'))])
            if change_basic_ids:
                raise ValidationError(_(u'Vendor已经有change流程中的单子！'))
            change_terms_ids = record.env['iac.vendor.change.terms'].search([('vendor_id', '=', record.vendor_id.id), (
                'state', 'in', ('to approve', 'unapproved', 'to sap', 'sap error'))])
            if change_terms_ids:
                raise ValidationError(_(u'Vendor已经有change流程中的单子！'))
    ...
    def button_to_approve(self):
        ...
        # vendor block/unblock/change流程中的不能送签表单
        vendor_block_ids = self.env['iac.vendor.block'].search(
            [('id', '!=', self.id), ('vendor_id', '=', self.vendor_id.id),
             ('state', 'in', ('to approve', 'unapproved', 'to sap', 'sap error'))])
        if vendor_block_ids:
        ...
        change_terms_ids = self.env['iac.vendor.change.terms'].search([('vendor_id', '=', self.vendor_id.id), (
            'state', 'in', ('to approve', 'unapproved', 'to sap', 'sap error'))])
        if change_terms_ids:
            raise UserError(_(u'Vendor已经有change流程中的单子，不能送签！'))
...
class IacVendorChangeBasic(models.Model):
    ...
    address_country = fields.Many2one('res.country', string="Address - Country Code *",
                                      domain=['&', ('show_in_list', '=', 'Y'), ('sh_import', '=', 'N')])
    currency = fields.Many2one('res.currency', string="Currency *")
    ...
    def _valid_record(self):
        ...
        #     vendor_register = self.env['iac.vendor.register'].search([('state', '!=', 'cancel'), ('short_name', '=', self.short_name),('id','!=',self.vendor_reg_id.id)], limit=1)
    ...
    def create(self, values):
        if values['vendor_reg_id']:
            ...
            vendor_block_ids = self.env['iac.vendor.block'].sudo().search(
                [('vendor_id', '=', vendor_reg_id.vendor_id.id), (
                    'state', 'in', ('to approve', 'to sap', 'sap error'))])
            if vendor_block_ids:
                raise UserError(_(u'Vendor已经有block/unblock流程中的单子，不能创建！'))
            change_basic_ids = self.env['iac.vendor.change.basic'].sudo().search(
                [('id', '!=', self.id), ('vendor_reg_id', '=', vendor_reg_id.id),
                 ('state', 'in', ('draft', 'to approve', 'to sap', 'sap error'))])
            if change_basic_ids:
                raise UserError(_(u'Vendor已经有change流程中的单子，不能创建！'))
        ...
        if not conf_obj.get('import_data_mode'):
            vendor_change_rec._valid_record()
    ...
    def write(self, values):
        ...
        change_basic = super(IacVendorChangeBasic, self).write(values)
        if not conf_obj.get('import_data_mode'):
            self._valid_record()
        return change_basic
    ...
    def button_to_approve(self):
        # vendor block/unblock/change流程中的不能送签表单
        vendor_block_ids = self.env['iac.vendor.block'].search([('vendor_id', '=', self.vendor_reg_id.vendor_id.id),
                                                                ('state', 'in', ('to approve', 'to sap', 'sap error'))])
        if vendor_block_ids:
            raise UserError(_(u'Vendor已经有block/unblock流程中的单子，不能送签！'))
        change_basic_ids = self.env['iac.vendor.change.basic'].search(
            [('id', '!=', self.id), ('vendor_reg_id', '=', self.vendor_reg_id.id),
             ('state', 'in', ('to approve', 'to sap', 'sap error'))])
        if change_basic_ids:
            raise UserError(_(u'Vendor已经有change流程中的单子，不能送签！'))
        ...
        if self.vendor_reg_id.vendor_id.vendor_type in ["normal"]:
            if self.attachment_ids:
                for attachment_id in self.attachment_ids:
                    # if attachment_id.type. == attachment_id.file_id:
                    if (attachment_id.type.special_approved == True) and (not attachment_id.approver_id.exists()) and \
                            (attachment_id.file_id):
                        raise UserError(
                            _(u'Attachment type is %s must set a approver !') % (attachment_id.type.name,))
            else:
                raise UserError(_(u'Attachment必须上传！/ Attachment is required to upload!'))
        ...
        if model_obj.effective_date and fields.Datetime.from_string(model_obj.effective_date) <= datetime.today():
            ...
            if rpc_result:
                ...
                basic_vars = {
                    ...
                    'currency': model_obj.currency.id  # 190222 ning add 更新币别
                }
                model_obj.vendor_reg_id.with_context({"no_check_short_name": True}).write(basic_vars)
                ...
                bank_vars = {
                    ...
                    'bank_city': model_obj.bank_city,  # 190222 ning add 更新city
...
class IacVendorChangeTerms(models.Model):
    ...
    file_description = fields.Char(string="File Description")  # 文件描述信息
    state = fields.Selection([
        ('draft', 'Draft'),  # buyer自己编辑保存的状态
        ('to approve', 'To Approve'),  # buyer review后提交webflow签核，送签
        ('unapproved', 'Unapproved'),  # webflow拒绝或抽单
    ...
    def button_to_approve(self):
        ...
        # vendor block/unblock/change流程中的不能送签表单
        vendor_block_ids = self.env['iac.vendor.block'].search([('vendor_id', '=', self.vendor_id.id), (
            'state', 'in', ('to approve', 'unapproved', 'to sap', 'sap error'))])
        if vendor_block_ids:
            raise UserError(_(u'Vendor已经有block/unblock流程中的单子，不能送签！'))
        change_basic_ids = self.env['iac.vendor.change.basic'].search(
        ...
        if rpc_result:
            self.write(
                {'state': 'to approve', 'webflow_number': rpc_json_data.get('EFormNO'), 'state_msg': u'webflow送签成功'})
            message = u'送签成功'
        else:
    ...
    def vendor_change_payment_incoterm_callback(self, context=None):
        ...
        try:
            ...
            if not context["approve_status"] or not context.get("data") or not context.get("data").get("id"):
                ...
                if context["approve_status"] and context["rpc_callback_data"]["FormStatus"] == "C":
                    # 修改state
                    model_obj = self.browse(int(context.get("data").get("id")))
                    if not model_obj.exists():
                        err_msg = "id is %s not exists in iac.vendor.change.terms " % (context.get("data").get("id"),)
                        proc_ex.append(err_msg)
                        _logger.error(err_msg)
                        proc_result = False
                        return proc_result, proc_ex
        ...
        try:
            ...
            if model_obj.effective_date and fields.Datetime.from_string(model_obj.effective_date) <= datetime.today():
                ...
                if rpc_result:
                    ...
                    # if att_type:  # ('type', '=', att_type.id)
                    # iac_vendor_att = self.env['iac.vendor.attachment'].search([('vendor_id', '=', model_obj.vendor_id.id)])
                    # if iac_vendor_att:
                    #     vars1 = {
...
class IacVendorChangeManufacture(models.Model):
    ...
    def button_confirm(self):
        ...
        if not self.name1_en or utility.contain_zh(self.name1_en):
            raise UserError(_(
                u'Company Name 1(English)不能为空且不能包含中文！/ Company Name 1(English) is required and Chinese character is not allowed!'))
        if not self.short_name or utility.contain_zh(self.short_name):
            raise UserError(_(
                u'Supplier Short Name(Only English)不能为空且不能包含中文！/ Supplier Short Name(Only English) is required and Chinese character is not allowed!'))
        if not self.contact_person:
            raise UserError(_(u'Contact Person不能为空！/ Contact Person is required!'))
        if not self.company_telephone1 or not utility.is_phone(self.company_telephone1):
            raise UserError(_(u'Company Tel. 1 必须填写有效的电话号码！/ Company Tel. 1 is invalid!'))
        ...
        if values.get('line_ids', False):
            for line_id in values.get('line_ids', False):
                if line_id[0] == 0:  # 新添加
                    ...
                    list_line_remove_ids.append(line_id)
            int_list_line_reserve_ids = [self.env['mail.message.white.line'].browse(x[1]).vendor_id.id for x in
                                         list_line_reserve_ids]
            for item in line_ids:
                if item not in int_list_line_reserve_ids:
...
class MailMessageWhiteUpload(models.TransientModel):
    ...
    def action_confirm(self):
        ...
        options = {u'datetime_format': u'', u'date_format': u'%Y-%m-%d', u'keep_matches': False, u'encoding': u'utf-8',
    ...
    def do(self, fields, options, dryrun=False):
        ...
        for line in data:
            # 补齐10位vendor code
            vendor = self.env['iac.vendor'].search(
                [('vendor_code', '=', line[0].zfill(10)), ('state', 'in', ('done', 'block'))], limit=1)
            if vendor:
