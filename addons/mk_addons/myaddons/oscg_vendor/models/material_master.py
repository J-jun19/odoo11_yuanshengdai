...
class PartStatus(models.Model):
    ...
    part_status = fields.Char(string='part status', index=True)
    ...
    def name_get(self):
        ...
        for status in self:
            ...
            if status.part_status:
                name = "%s(%s)" % (status.part_status, name)
...
class MaterialMaster(models.Model):
    ...
    miss_flag = fields.Integer(string="Miss Flag",default=0,index=True)
    #ning add 190904
    map = fields.Float(digits=(10,2))
    price_unit = fields.Float()
    #lz add part_status_id
    part_status_id = fields.Many2one('part.status', string="Part Status Id",index=True)
