...
class IacStorageLocationAddress(models.Model):
    _name = 'iac.storage.location.address'
    _rec_name = 'storage_location'

    plant = fields.Char(string='Plant', index=True)
    storage_location = fields.Char(string='Location Num', index=True)
    sloc_name = fields.Char(string='Location name', index=True)
    company_name = fields.Char(string='Company', index=True)
    sloc_address = fields.Char(string='Address', index=True)
    sloc_zip = fields.Char(string='ZIP', index=True)
    sloc_chi_name = fields.Char(string='Chinese Name', index=True)
    sloc_state = fields.Char(string='State', index=True)
    sloc_eng_name = fields.Char(string='Englist Name', index=True)
    sloc_country = fields.Char(string='Country', index=True)
    sloc_city = fields.Char(string='City', index=True)
    CM_VENDOR = fields.Char()  # 20190402 jun add
    ...
    def name_get(self):
        ...
        for location in self:
            ...
            if location.storage_location:
                name = "%s(%s)" % (location.storage_location, name)
