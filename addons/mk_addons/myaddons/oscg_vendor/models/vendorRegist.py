...
class IacVendorRegister(models.Model):
    ...
    def _valid_record(self):
        ...
        if "no_check_short_name" not in self._context and self.short_name != False:
            vendor_register = self.env['iac.vendor.register'].search(
                [('state', '!=', 'cancel'), ('short_name', '=', self.short_name), ('id', '!=', self.id)], limit=1)
    ...
    def button_registe(self):
        ...
        if not self.short_name:
            raise UserError(
                _(u'Supplier Short Name(Only English)不能为空！/ Supplier Short Name(Only English) is required!'))
        ...
        if not utility.is_digital(self.employee_number):
            raise UserError(
                _(u'Number of Employee不能为空且必须填写正整数！/ Number of Employee is required and has to be positive integer.'))
        ...
        if not utility.is_email(self.sales_email):
            raise UserError(
                _(u'Contact email不能为空且必须是有效的email！/ Contact email is required and has to be valid eamil address!'))
    ...
    def button_to_approve(self):
        ...
        # 上传文档对应的审核人和失效时间校验
        config_list = self.env['iac.attachment.config'].search(
            [('model_obj', '=', 'vendor'), ('is_displayed', '=', True)])
    ...
    def generate_account(self, name, email, password, buyer_email):
        ...
        if fields.Datetime.from_string(expire_date) > datetime.now():
            ...
            UserName = 'User Name：' + str(login)
            # 给user发送邮件
            try:
                UserName = 'User Name：' + str(login)
                subject = '[系統通知]  IAC Portal Login Info --' + str(name)
                total_mail = buyer_email + ';' + 'Wang.Ningg@iac.com.tw' + ';' + 'Zhang.Pei-Wu@iac.com.tw'
                self.env['iac.email.pool'].button_to_mail('iac-ep_support@iac.com.tw', email, total_mail, subject,
                                                          ['Login info'], [[
                                                                               'Following is your Login user and password for IAC Supplier Portal'],
                                                                           [
                                                                               'Please change your password upon first time login for security purpose'],
                                                                           [UserName], ['Password：********']],
                                                          'VENDOR_NEW')
            except:
                traceback.print_exc()
            ...
            return u"新用户注册成功，请注意查收邮件，以新账号登录系统,如長時間未收到郵件，請看下是否歸類到垃圾桶，仍然沒有可以聯絡IAC採購人員詢問。\n Please check your mailbox and use new account name to login,If you have not received the email for a long time, please contact the IAC buyer."
        else:
            return "System Error!"
            ...
            if not context["approve_status"] or not context.get("data") or not context.get("data").get("id"):
                ...
                if context["approve_status"] and context["rpc_callback_data"]["FormStatus"] == "C":
                    ...
                    if not vendor_reg.exists():
                        proc_result = False
                        proc_ex.append(
                            "iac_vendor_register with id %s is not exists!" % (context.get("data").get("id"),))
                        _logger.error(
                            "iac_vendor_register with id %s is not exists!" % (context.get("data").get("id"),))
                        return proc_result, proc_ex
                    ...
                    for file_val in file_list:
                        ...
                        file_attachment_rec = self.env["iac.vendor.register.attachment"].browse(file_id)
                        if not file_attachment_rec.exists():
                            err_msg = "id is %s not exists in iac.vendor.register.attachment " % (file_id,)
                    ...
                    vendor_reg.write(
                        {'state': 'done', 'state_msg': u'webflow(%s)签核通过' % context["rpc_callback_data"]["EFormNO"]})
                    ...
                    vendor_vals = {
                        ...
                        'site_survey': context.get("rpc_callback_data", {}).get("params", {}).get("sitesurvey", False),
                        'comment': str(context.get("rpc_callback_data", {}).get("params", {}).get("Comments", [])),
                        # 'comment': str(context.get("data").get("Comments")),
                        'attachment_ids': attachment_ids,
                        'vendor_type': 'normal',
                    ...
                    vendor_reg.write({'state': 'unapproved',
...
class IacAttachment(models.Model):
    ...
    group = fields.Selection(string='Group', selection='_selection_subgroup', required=True,
                             default="basic")  # basic：基本资料的附件；bank：银行资料的附件
...
class IacVendorRegisterAttachment(models.Model):
    ...
    name1_cn = fields.Char('Name CN', related='vendor_reg_id.name1_cn')
    name1_en = fields.Char('Name EN', related='vendor_reg_id.name1_en')
    supplier_type = fields.Selection('Supplier Type', related='vendor_reg_id.supplier_type')
    buyer_email = fields.Char('Buyer Email', related='vendor_reg_id.buyer_email')
    ...
    def write(self, values):
        ...
        if values.get('file_id', False):
            ...
            utility.send_to_email(self, self.id, 'oscg_vendor.vendor_register_qs_attachment_email')
        # ning add 181204  upload_date字段的值给上传当天
        values['upload_date'] = datetime.now().strftime('%Y-%m-%d')
    ...
    @api.model
    def check_file_link(self, dms_file_id):
        """
        检查是否允许进行文件链接操作,只检查External vendor 这一个组
        文件是否在iac.vendor.register.attachment 这个模型中存储
        :param dms_file_id:
        :return:
        """
        file_group = self.env.ref('oscg_vendor.IAC_vendor_groups')
        if self.env.user.id not in file_group.users.ids:
            return True
        domain = [('file_id', '=', dms_file_id)]
...
class IacVendorAttachment(models.Model):
    ...
    approver_id = fields.Many2one('res.users', string='Approve User')
    name1_cn = fields.Char('Name CN', related='vendor_id.vendor_reg_id.name1_cn')
    name1_en = fields.Char('Name EN', related='vendor_id.vendor_reg_id.name1_en')
    supplier_type = fields.Selection('Supplier Type', related='vendor_id.vendor_reg_id.supplier_type')
    buyer_email = fields.Char('Buyer Email', related='vendor_id.vendor_reg_id.buyer_email')
...
# 继承vendor表，用来跳过对buyer角色的权限
class IacVendorInherit(models.Model):
    _name = 'iac.vendor.inherit'
    _inherit = 'iac.vendor'
    _table = 'iac_vendor'
...
class IacVendorAttachmentWizard(models.TransientModel):
    ...
    vendor_id = fields.Many2one('iac.vendor.inherit', string="Vendor")
    state = fields.Selection([
        ('upload', 'Upload'),
        ('active', 'Activate'),
        ('inactive', 'Inactivate')
        ...
        for wizard in self:
            ...
            if wizard.vendor_id:
                for item in self.env.user.groups_id:
                    if item.name == 'File reviewer':
                        is_file_reviewer = 1
                        break
                    else:
                        continue
                if is_file_reviewer == 1:
                    domain += [('vendor_id', '=', wizard.vendor_id.id)]
                else:
                    buyer_email = self.env['iac.vendor.inherit'].browse(wizard.vendor_id.id).buyer_email
                    if buyer_email == self.env.user.partner_id.email:
                        domain += [('vendor_id', '=', wizard.vendor_id.id)]
                    else:
                        raise UserError("当前vendor对应的采购邮箱与当前操作人的采购邮箱不符，无法查询！")
            else:
                for item in self.env.user.groups_id:
                    if item.name == 'File reviewer':
                        is_file_reviewer = 1
                        ...
                        continue
                if is_file_reviewer == 0:
                    vendor_id_list = []
                    vendor_objs = self.env['iac.vendor.inherit'].search(
                        [('buyer_email', '=', self.env.user.partner_id.email)])
                    for vendor in vendor_objs:
                        vendor_id_list.append(vendor.id)
                    domain += [('vendor_id', 'in', vendor_id_list)]
            if wizard.state:
...
# 继承register表，用来跳过对buyer角色的权限
class IacVendorRegisterInherit(models.Model):
    _name = 'iac.vendor.register.inherit'
...
class IacVendorRegisterAttachmentWizard(models.TransientModel):
    ...
    vendor_reg_id = fields.Many2one('iac.vendor.register.inherit', string="Vendor Register")
    state = fields.Selection([
        ('upload', 'Upload'),
        ('active', 'Activate'),
        ('inactive', 'Inactivate')
        ...
        for wizard in self:
            ...
            if wizard.vendor_reg_id:
                ...
                if is_file_reviewer == 1:
                    ...
                    buyer_email = self.env['iac.vendor.register.inherit'].browse(wizard.vendor_reg_id.id).buyer_email
                    if buyer_email == self.env.user.partner_id.email:
                        domain += [('vendor_reg_id', '=', wizard.vendor_reg_id.id)]
                    else:
                        raise UserError("当前vendor对应的采购邮箱与当前操作人的采购邮箱不符，无法查询！")
                ...
                if is_file_reviewer == 0:
                    vendor_reg_id_list = []
                    vendor_reg_objs = self.env['iac.vendor.register.inherit'].search(
                        [('buyer_email', '=', self.env.user.partner_id.email)])
                    for vendor_reg in vendor_reg_objs:
                        vendor_reg_id_list.append(vendor_reg.id)
                    domain += [('vendor_reg_id', 'in', vendor_reg_id_list)]
...
class IacVendorFactory(models.Model):
    ...
    factory_type = fields.Selection(
        [("NORMAL", u"NORMAL"), ("FOUNDRY", u"FOUNDRY"), ("PACKAGING", u"PACKAGING"), ("TESTING", u"TESTING")],
        String="Type")
    ...
    relation = fields.Selection(
        [("Self-Owned", u"Self-Owned"), ("Joint-Venture", u"Joint-Venture"), ("Outsourcing", u"Outsourcing")],
        string="Relation")
...
class IacVendorMaterial(models.Model):
    ...
    material_group = fields.Many2one('material.group', string="Material Group*",
                                     domain=[('material_group', 'ilike', 'R-')])
