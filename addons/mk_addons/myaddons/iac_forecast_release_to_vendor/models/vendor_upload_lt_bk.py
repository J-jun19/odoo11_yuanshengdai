...
class VendorUploadLTBK(models.TransientModel):
    # vendor 上傳LT的程式。選擇上傳檔案路徑的 wizard table
    _name = 'iac.vendor.upload.lt.wizard.bk'
    ...
    def action_confirm(self):
        ...
        if sheet_obj.cell(0, 0).value <> 'BuyerName' or sheet_obj.cell(0, 158).value <> 'Version ID':
        ...
        w1_r_start_type = str(sheet_obj.cell(0,9)).split(':')[0]

        if w1_r_start_type=='xldate':
            w1_r_start = xlrd.xldate.xldate_as_datetime(sheet_obj.cell(0, 9).value, 0)
        else: # text
            w1_r_start = sheet_obj.cell(0, 9).value
        # print '*43:', w1_r_start_type ,'。',w1_r_start

        # 讀取AG~FB欄 所有日期,並寫入array裡
        header_date = []

        for i in range(32, 157):
            type = ''
            title = ''

            if i in [53,61,69,77,85,93,101,109,117,125,133,141,149,151,153,155,157]: # 這幾欄不讀
                title=''
            else:
        ...
        for rx in range(sheet_obj.nrows):
            ...
            if rx >= 3 and sheet_obj.cell(rx, 0).value:
                is_exist = 0
                if rx == 3:
                    vendor_list.append(sheet_obj.cell(rx, 159).value)
                    material_list.append(sheet_obj.cell(rx, 1).value)
                    storage_location_list.append(sheet_obj.cell(rx,162).value)
                else:
                    for i in range(len(vendor_list)):
                        if sheet_obj.cell(rx, 159).value == vendor_list[i] \
                                and sheet_obj.cell(rx, 1).value == material_list[i]\
                                and sheet_obj.cell(rx,162).value == storage_location_list[i]:
                            is_exist = 1
                            first_exist = i
                ...
                if sheet_obj.cell(rx,158).value == '':
                    version_id = ''
                else:
                    version_id = str(sheet_obj.cell(rx, 158).value)  # version_id = str(int(sheet_obj.cell(rx, 158).value))
                ...
                if not self.env['iac.storage.location.address'].search(
                        [('plant', '=', sheet_obj.cell(rx, 161).value.upper()),
                         ('storage_location', '=', sheet_obj.cell(rx, 162).value.upper())]):
                    storage_location_id = ''
                    ...
                    storage_location_id = self.env['iac.storage.location.address'].search(
                        [('plant', '=', sheet_obj.cell(rx, 161).value.upper()),
                         ('storage_location', '=', sheet_obj.cell(rx, 162).value.upper())]).id

                if sheet_obj.cell(rx,1).value == '':
                    material_code = ''
                else:
                    material_code = str(sheet_obj.cell(rx, 1).value)
                if sheet_obj.cell(rx, 159).value == '':
                    vendor_code = ''
                    iac_pn_vendor = ''
                else:

                    vendor_code = str(int(sheet_obj.cell(rx, 159).value)).zfill(10)  # 補 0   #str(int(sheet_obj.cell(rx, 159).value))
                    iac_pn_vendor = str(sheet_obj.cell(rx, 1).value) + str(sheet_obj.cell(rx, 159).value)
                ...
                buyer_code = str(sheet_obj.cell(rx, 0).value[:3])  # str(int(sheet_obj.cell(rx, 0).value[:3])) # A  0
                if sheet_obj.cell(rx, 2).value == '' or (str(sheet_obj.cell(rx, 2).value).upper() != 'Y' and str(
                        sheet_obj.cell(rx, 2).value).upper() != 'N'):
                    error_num4 = error_num4 + str(rx + 1) + ' , '
                    error_count =  1
                    key_part = ''
                    # raise exceptions.ValidationError("Key Part 欄位必需為'Y'或'N' !")
                else:
                    key_part = str(sheet_obj.cell(rx, 2).value).upper()  # C  2

                for i in range(32, 157):
                    if i in [53, 61, 69, 77, 85, 93, 101, 109, 117, 125, 133, 141, 149, 151, 153, 155, 157]:
                        # print i
                        continue
                    else:
                        # print i
                ...
                vendor_remark = sheet_obj.cell(rx, 31).value  # AF
