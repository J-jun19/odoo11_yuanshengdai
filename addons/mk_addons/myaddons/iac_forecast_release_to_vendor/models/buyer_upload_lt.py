...
class BuyerUploadLT(models.TransientModel):
    # buyer 上傳LT的程式。選擇上傳檔案路徑的 wizard table
    _name = 'iac.buyer.upload.lt.wizard'
    ...
    def action_confirm_buyer_upload_lt(self):
        ...
        first_title = self.env['iac.buyer.fcst.setting'].search([],order='column_begin',limit=1).title
        end_field = self.env['iac.buyer.fcst.setting'].search([],order='column_end desc',limit=1)
        ...
        error_count = 0
        #抓取日期字段的起始列和结束列
        date_begin_column = self.env['iac.buyer.fcst.setting'].search([('interval','!=',0)],order='column_begin',limit=1).column_begin
        date_end_column = self.env['iac.buyer.fcst.setting'].search([('interval','!=',0)],order='column_end desc',limit=1).column_end
        # 先判斷 excel cell的格式 是 date 還是 text
        w1_r_start_type = str(sheet_obj.cell(2,date_begin_column)).split(':')[0]

        # print '*48:', w1_r_start_type ,',',sheet_obj.cell(2,11)

        if w1_r_start_type == 'xldate':
            w1_r_start = xlrd.xldate.xldate_as_datetime(sheet_obj.cell(2, date_begin_column).value, 0)
        else: # text
            w1_r_start = sheet_obj.cell(2, date_begin_column).value
        # print '*43:', w1_r_start_type ,'。',w1_r_start

        # 讀取AG~FB欄 所有日期,並寫入array裡
        header_date = []
        #range取不到最后一位，所以最后一列要加1
        for i in range(date_begin_column, date_end_column+1)  :
            type = ''
            title = ''
        ...
        vendor_column = self.env['iac.buyer.fcst.delivery.report.wizard'].get_begin_column_by_name('廠商代碼','iac.buyer.fcst.setting')
        material_column = self.env['iac.buyer.fcst.delivery.report.wizard'].get_begin_column_by_name('料號','iac.buyer.fcst.setting')
        location_column = self.env['iac.buyer.fcst.delivery.report.wizard'].get_begin_column_by_name('Location','iac.buyer.fcst.setting')
        plant_column = self.env['iac.buyer.fcst.delivery.report.wizard'].get_begin_column_by_name('Plant','iac.buyer.fcst.setting')
        buyer_column = self.env['iac.buyer.fcst.delivery.report.wizard'].get_begin_column_by_name('採購代碼','iac.buyer.fcst.setting')
        key_part_column = self.env['iac.buyer.fcst.delivery.report.wizard'].get_begin_column_by_name('Key Part','iac.buyer.fcst.setting')
        remark_column = self.env['iac.buyer.fcst.delivery.report.wizard'].get_begin_column_by_name('Remark','iac.buyer.fcst.setting')
        for rx in range(sheet_obj.nrows):
            ...
            if rx >= 3 and sheet_obj.cell(rx, 0).value:
                ...
                if not self.env['iac.storage.location.address'].search([('plant','=',sheet_obj.cell(rx,plant_column).value.upper()),('storage_location','=',sheet_obj.cell(rx,location_column).value.upper())]):
                    storage_location_id = ''
                ...
                if sheet_obj.cell(rx, vendor_column).value == '':
                    ...
                    vendor_code = str(int(sheet_obj.cell(rx, vendor_column).value)).zfill(10)  # 補 0
                    iac_pn_vendor = str(sheet_obj.cell(rx, material_column).value) + str(int(sheet_obj.cell(rx, vendor_column).value))
                if sheet_obj.cell(rx, buyer_column).value == '':
                    buyer_code = ''
                else:
                    buyer_code = str(int(sheet_obj.cell(rx, buyer_column).value))  # A  0
                if sheet_obj.cell(rx,key_part_column).value =='' or (str(sheet_obj.cell(rx,key_part_column).value).upper() != 'Y' and str(sheet_obj.cell(rx,key_part_column).value).upper() !='N'):
        ...
        for rx in range(sheet_obj.nrows):  # range(0, 6):    temp  for test 先讀到第6行
            ...
            if rx >= 3 and error_count==0 and sheet_obj.cell(rx, 0).value: # 從第4行開始讀
                ...
                if key_part.upper()=='N' and error_count==0:
                    ...
                    #shipping_date = xlrd.xldate.xldate_as_datetime(sheet_obj.cell(2, date_begin_column).value, 0)
		    shipping_date = sheet_obj.cell(2,date_begin_column).value
                ...
                if error_count == 0: #沒錯誤再寫入
                    sum = 0
                    for j in range(date_begin_column, date_end_column+1):
                    ...
                    for i in range(date_begin_column, date_end_column+1):
                        ...
                        if sheet_obj.cell(rx, i).value >= 0 and sheet_obj.cell(rx, i).value !='' and isinstance(sheet_obj.cell(rx, i).value,(float)):
                            ...
                            demand_date = header_date[i-date_begin_column]
