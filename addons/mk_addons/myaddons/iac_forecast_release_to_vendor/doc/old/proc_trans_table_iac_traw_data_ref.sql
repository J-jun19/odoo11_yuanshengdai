-- FUNCTION: ep_temp_master.proc_trans_table_iac_traw_data_ref(character varying, integer, integer, integer, integer)

-- DROP FUNCTION ep_temp_master.proc_trans_table_iac_traw_data_ref(character varying, integer, integer, integer, integer);
/*
...
*/
CREATE OR REPLACE FUNCTION ep_temp_master.proc_trans_table_iac_traw_data_ref(
...
for v_rc in select * from "public".iac_traw_data where sap_log_id=v_sap_log_id   order by id LIMIT v_limit offset v_offset                                                                                            
loop                                                                                                                                                              
			begin
          ...
          v_table_name:='iac_traw_data';                                                                                                                                 
					v_exception_detail:='A error occurs when insert table 1 ( '||v_table_name||' ),source id is ( '||v_rc.id||' )';                                  
					v_src_id=v_rc.id;                                                                                                                              
...
ALTER FUNCTION ep_temp_master.proc_trans_table_iac_traw_data_ref(character varying, integer, integer, integer, integer)
    OWNER TO openerp;
