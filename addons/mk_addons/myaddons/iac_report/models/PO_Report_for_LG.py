...
class IacVendorRegisterPOReportForLG(models.Model):
    _inherit = 'iac.vendor.register'
    _name = 'iac.vendor.register.po.report.for.lg'
    _table = 'iac_vendor_register'


class IacPOReportForLG(models.Model):
    _inherit = 'iac.purchase.order'
    _name = 'iac.po.report.for.lg'
    ...
    vendor_reg_id = fields.Many2one('iac.vendor.register.po.report.for.lg')
    # vendor_code = fields.Char(string='Vendor Code', related='vendor_id.vendor_code', readonly=True)


class IacPOReportForLGWizard(models.TransientModel):
    _name = 'v.po.report.for.lg.wizard'
    ...
    document_erp_id = fields.Text(string='PO')
    ...
    @api.multi
    def search_iac_po_report_for_lg(self):
        # for item in self.env.user.groups_id:
        ...
        for wizard in self:
            ...
            if wizard.document_erp_id:
                document_erp_list = wizard.document_erp_id.split('\n')
                domain += [('document_erp_id', 'in', document_erp_list)]
            ...
            result = self.env['iac.po.report.for.lg'].search(domain)

        action = {
            'domain': [('id', 'in', [x.id for x in result])],
            'name': _('IAC PO Report for LG'),
            'type': 'ir.actions.act_window',
            'view_mode': 'tree,form',
            'res_model': 'iac.po.report.for.lg'
        }
        return action
