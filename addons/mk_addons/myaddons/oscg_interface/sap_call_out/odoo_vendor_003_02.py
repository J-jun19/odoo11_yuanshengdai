...
import traceback
class WebCallOutSapVendor00302(models.AbstractModel):
    _name = 'web.call.out.sap.vendor.003.02'
    _inherit="sap.call.out.base"
    _description = "Web Call Out Webflow F03 Interface"
    ...
    def invoke_web_call_with_log(self,context=None):
        ...
        exception_log=[]
        log_line_id=0
        json_data,exception_log=super(WebCallOutSapVendor00302,self).generate_json(context)
        context["json_params"]=json_data
        rpc_result,rpc_data,log_line_id,exception_log=super(WebCallOutSapVendor00302,self).invoke_web_call_with_log(context)
    ...
    def process_callback_func(self,context):
        ...
        try:
            ...
            #result_dict["vendor_code"]=context.get("Document").get("VENDOR")
        except:
            traceback.print_exc()
