...
class ir_cron_subclass(models.Model):
    ...
    def __init__(self, pool, cr):
        cr.execute("select buyer_name,buyer_erp_id from \"public\".buyer_code")
        res=cr.fetchall()
        print 'i has init ed'
    ...
    def write(self, vals):
        self._try_lock()
        return super(ir_cron_subclass, self).write(vals)

    @api.multi
    def unlink(self):
        self._try_lock()
        return super(ir_cron_subclass, self).unlink()
    ...
    def try_write(self, values):
        try:
            ...
            return super(ir_cron_subclass, self).write(values)
        return False
