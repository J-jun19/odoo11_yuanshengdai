...
class IacTransGroupAsn(models.TransientModel):
    _name="iac.trans.group.asn"
    ...
    def proc_update_ref(self,sap_log_id):
        ...
        select_count="select coalesce(max(t2.record_count),0) from (                                                              " \
                     "select count(*) record_count from ep_temp_master.iac_asn t where t.sap_log_id='%s'                 " \
                     "union                                                                                                       " \
                     "select count(*) record_count from ep_temp_master.iac_asn_line t where t.sap_log_id='%s') t2 " \
                     %(sap_log_id,sap_log_id,)
        ...
        if record_count==0:
            _logger.debug('no record found in iac_asn ')
            return

        #获取组合租条目的执行日志记录对象
        group_log_rec,map_group_line_log_rec=self._get_group_line_log(sap_log_id,"TRANS_ASN")
        ...
        for offset_count in page_list:
            sql_text="select * from ep_temp_master.sp_asn_group_update_ref('%s',%s,%s)  " \
            ...
            self.env.cr.execute("select * from ep_temp_master.sp_asn_group_update_ref(%s,%s,%s)  " \
                                "as (last_id int4,group_id int4,group_line_id int4 ,group_name varchar,group_line_name varchar,update_count int4,fail_count int4,miss_count int4)",
                                (sap_log_id, limit_count, offset_count))
            for last_id,group_id,group_line_id,group_name,group_line_name, update_count, fail_count,miss_count in self.env.cr.fetchall():
                ...
                _logger.debug("executing db func %s ,update records:%s,fail records:%s,miss records:%s," % (
                    "sp_asn_group_update_ref", update_count, fail_count,miss_count))
    ...
    def proc_update_ref_miss(self,sap_log_id):
        ...
        record_count = record_count_result[0][0]
        if record_count==0:
            _logger.debug('no record found in ep_temp_master.iac_asn ')
            return
    ...
    def proc_trans_prod(self,sap_log_id):
        ...
        select_count="select count(*) record_count from ep_temp_master.iac_asn t where t.sap_log_id='%s'                 " \
                     %(sap_log_id,sap_log_id,)
        self.env.cr.execute(select_count)
        record_count_result = self.env.cr.fetchall()
        ...
        miss_count_sum=0
        for offset_count in page_list:
            sql_text="select * from ep_temp_master.sp_asn_group_trans_prod('%s',%s,%s)  " \
                     "as (last_id int4,group_id int4,group_line_id int4 ,group_name varchar,group_line_name varchar,update_count int4,fail_count int4,miss_count int4)" \
                     % (sap_log_id, limit_count, offset_count)
            _logger.debug(sql_text)

            self.env.cr.execute("select * from ep_temp_master.sp_asn_group_trans_prod(%s,%s,%s)  " \
                                "as (last_id int4,group_id int4,group_line_id int4 ,group_name varchar,group_line_name varchar,update_count int4,fail_count int4,miss_count int4)",
                                (sap_log_id, limit_count, offset_count))
            for last_id,group_id,group_line_id,group_name,group_line_name, update_count, fail_count,miss_count in self.env.cr.fetchall():
                miss_count_sum = miss_count_sum + miss_count
                ...
                fail_count_sum = fail_count_sum + fail_count
                _logger.debug("executing db func %s ,update records:%s,fail records:%s,miss records:%s," % (
                    "sp_asn_group_trans_prod", update_count, fail_count,miss_count))
                log_line_rec=map_group_line_log_rec.get(group_line_id)
                if log_line_rec !=None:
                    log_line_vals={
                        "last_id":last_id,
